...
if(isset($_POST['usersave'])&&($_POST['password']==$_POST['vpassword']))
{
	...
	if($_POST['id']!='')
	{
            ...
            if($usrcount>='1')
            {
		$this->dbupdate('users',$loginsave,"sourceid='".$_POST['id']."' AND usertype='instructor'");
	...
	$this->loadextraClass('registration');
        ...
        $user = $this->runquery("SELECT * FROM contributors WHERE contributorid='".$_POST['id']."' AND category = 'Instructor'",'single');
