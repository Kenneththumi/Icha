...
<!----- header div end ------->
	...
	<div class="span12">	    
		...
		foreach ($files as $file_array) {
			...
			if($file == '.' 
                                || (isset($file_array['extension']) && $file_array['extension']!=lang_Type_dir) 
                                || ($file == '..' && $subdir == '') 
                                || in_array($file, $hidden_folders) 
                                || ($filter!='' && $file!=".." && strpos($file,$filter)===false))
			    continue;
